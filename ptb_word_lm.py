...
class PTBInput(object):
  ...
  def __init__(self, config, data, name=None, iter=0):
    self.batch_size = batch_size = config.batch_size
    self.num_steps = num_steps = config.num_steps
    if (name == 'TrainInput'):
      self.epoch_size = (((len(data['EAP'])+len(data['MWS'])+len(data['HPL'])) // batch_size) - 1) // num_steps
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    if FLAGS.load_path:
      ...
      with tf.name_scope("Train"):
        ...
        with tf.variable_scope("Model", reuse=None):
            m = PTBModel(is_training=True, config=config, input_=train_input, name="Train")
        tf.summary.scalar("Training Loss", m.cost)
        tf.summary.scalar("Learning Rate", m.lr)                                            
      with tf.name_scope("Test"):
        test_input = PTBInput(
            config=eval_config, data=test_data, name="TestInput")#,iter=0)
    ...
    models = {"Train": m, "Test": mtest}#, "Valid": mvalid, "Test": mtest}
